...
void GetRotationVector::getRotation(const std::string& filename)
{
    ...
    for(int i=0; i<12; i++){
        ...
        rotation_vec.push_back(std::stof(param));
    }
    ...
    **/
    ...
    R(0,1) = rotation_vec[1];
    R(0,2) = rotation_vec[2];
 
    R(1,0) = rotation_vec[3];
    R(1,1) = rotation_vec[4];
    R(1,2) = rotation_vec[5];
 
    R(2,0) = rotation_vec[6];
    R(2,1) = rotation_vec[7];
    R(2,2) = rotation_vec[8];

    R(0,3) = rotation_vec[9]; 
    R(1,3) = rotation_vec[10]; 
    R(2,3) = rotation_vec[11];
...
void GetRotationVector::transformPointCloud(int number) {
    ...
    std::string filename = "/mnt/container-data/katori_0906/ply_data/after/enable_rotation/for_regi/TS_1_2/matrix" 
                         + std::to_string(number-1) + "_" 
